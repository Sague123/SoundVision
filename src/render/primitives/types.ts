...
export interface RenderFrame {
  ...
  weight: number;
  /**
   * Какая доля слоя стирается за кадр, 0..1.
   *
   * Слой со следами копит: вклад `a` за кадр при затухании `fade` даёт
   * установившуюся яркость `a / fade`. Тонким линиям это безразлично, а
   * примитив, заливающий весь кадр, из-за этого выгорает в ровную муть.
